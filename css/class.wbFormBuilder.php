...
// type="image"
    ...
    function __construct ( $options = array() ) {
        ...
        elseif ( isset( $options['uselang'] ) ) {
            $this->lang          = new wbI18n( $options['uselang'] );
        ...
        if (
             isset( $options['asTable'] )
             &&
             $options['asTable'] === false
        ) {
            $this->_type = 'fieldset';
        }
    ...
    /**
  	 * generate a form, giving the options as an array
  	 *
  	 * @param array $options
  	 ...
  	 *
  	 **/
    public function generateFromArray( $options, $current = array() ) {
    ...
    }   // end function generateFromArray()
...
# ---------- TODO: validate $prefix -----
    ...
    public function password ( $options ) {
  	 ...
  	 * @param   string   on             field value
  	 *                                  (value sent when checkbox is marked)
  	 * @param   string   label          label
  	 *                                  (text to show)
  	 * @param   string   value          current value
  	 *                                  (used to decide if checkbox has to be
  	 *                                  checked)
  	 * @param   array    options        to create more than one checkbox
  	 *
    ...
    public function checkbox( $options ) {
        ...
        $checked = ( isset( $options['checked'] ) && is_array( $options['checked'] ) )
                 ? $options['checked']
                 : ( isset( $options['checked'] ) ? array( $options['checked'] => true ) : array() );

echo "<textarea cols=\"100\" rows=\"20\" style=\"width: 100%;\">OPT:\n";
var_export( $opt );
echo "</textarea>";

### ---- ACHTUNG KAPUTT! $key / $label wird gar nicht benutzt... ----
    ...
    public function radio( $options ) {
        ...
        $opt     = ( isset( $options['options'] ) && is_array( $options['options'] ) )
                 ? $options['options']
                 : array( $options['name'] => $this->lang->translate( $label ) );
        ...
        foreach ( $opt as $key => $label ) {
            ...
            if ( $isIndexed ) { $key = $label; }

            $options['checked']
                 = ( isset( $options['value'] ) && ! strcasecmp( $options['value'], $key ) )
                 ? 'checked'
                 : '';
            ...
            $output .= $this->__createInput(
                           array_merge(
                               $options,
                               array(
                                   'label' => $this->lang->translate( $label ),
                                   'value' => $key
                               )
                           )
                       );
     ...
     **/
        ...
        foreach ( $this->_elements as $elem ) {
            ...
            if ( $elem['name'] == '__legend__' ) {
                ...
                $req = isset( $this->_required[ $elem['name'] ] )
                     ? '*'
                     : NULL;
        ...
        // header
        $form_header = isset( $options['header'] )
                     ? $this->lang->translate( $options['header'] )
                     : '';
			  ...
			  $temp = $this->_tpl->parseString(
                    ...
                    array_merge(
                        ...
                        array (
                            ...
                            'info'
                                => ( isset($options['info']) ? $options['info'] : NULL ),
                            'infoclass'
